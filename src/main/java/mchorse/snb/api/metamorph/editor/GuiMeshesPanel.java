...
import mchorse.snb.api.animation.AnimationMeshConfig;
...
public class GuiMeshesPanel extends GuiMorphPanel<AnimatedMorph, GuiAnimatedMorph>
{
    ...
    public GuiMeshesPanel(Minecraft mc, GuiAnimatedMorph editor)
    {
        ...
        this.texture = new GuiButtonElement(mc, IKey.str("Pick texture"), (b) ->
        {
            this.picker.refresh();
        ...
        this.filtering = new GuiToggleElement(mc, IKey.str("Linear"), false, (b) ->
        {
            this.config.filtering = b.isToggled() ? GL11.GL_LINEAR : GL11.GL_NEAREST;
            this.editor.updateMorph();
        });

        this.normals = new GuiToggleElement(mc, IKey.str("Show normals"), false, (b) ->
        {
            this.config.normals = b.isToggled();
            this.editor.updateMorph();
        });

        this.smooth = new GuiToggleElement(mc, IKey.str("Smooth shading"), false, (b) ->
        {
            this.config.smooth = b.isToggled();
            this.editor.updateMorph();
        });

        this.visible = new GuiToggleElement(mc, IKey.str("Visible"), false, (b) ->
        {
            this.config.visible = b.isToggled();
            this.editor.updateMorph();
        });

        this.lighting = new GuiToggleElement(mc, IKey.str("Lighting"), false, (b) ->
        {
    ...
    public void draw(GuiContext context)
    {
        ...
        this.font.drawStringWithShadow("Meshes", this.configs.area.x, this.configs.area.y - 12, 0xffffff);

        if (this.fields.isVisible())
        {
            this.font.drawStringWithShadow("Texture", this.texture.area.x, this.texture.area.y - 12, 0xffffff);
            this.font.drawStringWithShadow("Color", this.color.area.x, this.color.area.y - 12, 0xffffff);
        }
